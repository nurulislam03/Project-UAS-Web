<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Prestasi Siswa') }}
        </h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="py-4 px-4 bg-white overflow-hidden shadow-sm sm:rounded-lg">
                
                <table class="table table-bordered">
                    <thead>
                        <tr>
                            <th scope="col" class="text-sm font-medium text-gray-900 px-6 py-4">ID</th>
                            <th scope="col" class="text-sm font-medium text-gray-900 px-6 py-4">Nama</th>
                            <th scope="col" class="text-sm font-medium text-gray-900 px-6 py-4">Kelas</th>
                            <th scope="col" class="text-sm font-medium text-gray-900 px-6 py-4">Prestasi</th>
                            <th scope="col" class="text-sm font-medium text-gray-900 px-6 py-4">Tingkat</th>
                            <th scope="col" class="text-sm font-medium text-gray-900 px-6 py-4">Edit</th>
                            <th scope="col" class="text-sm font-medium text-gray-900 px-6 py-4">Delete</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($prestasi as $prestasi)
                        <tr>
                            <td>{{ $prestasi->id }}</td>
                            <td>{{ $prestasi->nama }}</td>
                            <td>{{ $prestasi->kelas }}</td>
                            <td>{{ $prestasi->prestasi }}</td>
                            <td>{{ $prestasi->tingkat }}</td>
                            <td>
                                <a href="{{ url('/edit-prestasi/'.$prestasi->id)}}" class="btn btn-primary">Edit</a>
                            </td>
                            <td>
                                <form action="{{ url('delete-prestasi/'.$prestasi->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button class="btn btn-danger">Delete</button>
                                </form>
                            </td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="7"> No Record Found</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>

            </div>
        </div>
    </div>
</x-app-layout>
